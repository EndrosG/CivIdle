...
export function calculateTierAndPrice(log?: (val: string) => void) {
               ...
               if (!Config.MaterialTier[res] || targetTier < Config.MaterialTier[res]!) {
                  ...
                  forEach(resourceTierDependency, (k, v) => {
                     if (v === res) {
                        ...
                        log?.(
                           `Resource Tier of ${k} is decided by ${res}, but its tier has changed from ${oldTier ?? "?"
                           } to ${targetTier}`,
                        );
                     }
                  });
                  forEach(buildingTierDependency, (k, v) => {
                     if (v === res) {
                        ...
                        log?.(
                           `Building Tier of ${k} is decided by ${res}, but its tier has changed from ${oldTier ?? "?"
                           } to ${targetTier}`,
                        );
                     }
                  });
   ...
   forEach(Config.BuildingTier, (building) => {
      // Lydia @ 2026-04-17: TEST ... show real tier of producing wonders like Cloud Services
      // ... Consequence: construction menu does not show them in the wonder filter anymore but in the respective tier then
      // ... Do I really want this?
      if (isSpecialBuilding(building) && isEmpty(Config.Building[building].output)) {
